...
В проекте различаются три связанных, но не тождественных понятия:

- **Интерполяция** — математический *метод* оценки значений целевой переменной в неизвестных пространственных точках на основе наблюдаемых: kriging, IDW, kNN, GBM с пространственными признаками и т.п. Это основа, на которой строятся оба прикладных сценария ниже.

- **Импутация** — *прикладной use case* интерполяции: восстановление пропусков (NaN) в **существующих** строках датасета — там, где координаты известны, а target отсутствует. Реализуется через классы `Imputer`. Метрика качества — MAE / RMSE по восстановленным значениям.

- **Аугментация** — конкретно в нашем решении: использование значений, предсказанных интерполяцией в местах пропусков, как **дополнительных обучающих точек** для downstream-модели. То есть классификатор обучается дважды: (1) только на наблюдаемой части датасета — `F1(original)`; (2) на наблюдаемой части плюс восстановленные интерполяцией значения — `F1(augmented)`. Метрика полезности `ΔF1 = F1(augmented) − F1(original)` показывает, помогает ли интерполированное восстановление обучению, или вредит (типичный пример вреда: кригинг сглаживает редкие классы и может ухудшить точность на них). Реализуется через классы `Augmenter`.

Другие стратегии аугментации (генерация новых точек на регулярной сетке, балансирование плотности в разреженных зонах, mixup пространственных точек) — возможные расширения `Augmenter`, в первой версии работы не охвачены, остаются в backlog.

В эксперименте E1 импутация и аугментация связаны общим конвейером: на пропусках в датасете запускаются интерполяционные методы (это импутация); затем те же восстановленные значения добавляются к наблюдаемой части и подаются в обучение downstream-классификатора (это аугментация). Один прогон даёт сразу две оценки — качество восстановления (MAE) и downstream-пользу (ΔF1).
...
- Block Kriging для areal data. Backend выбирается по типу таргета: `pyinterpolate` имеет известное ограничение — корректно работает только с категориальными / счётными переменными (например, `kkt_count`), для непрерывных целевых (`avg_bill`, `median_bill`) используется `gstools`. Конкретный выбор проверяется на тестовом подмножестве ФНС до основных экспериментов
...
Также по каждой паре `(method, region)` фиксируются `fit_time_sec` и `transform_time_sec`. По log-log регрессии time vs N точек восстанавливается эмпирическая асимптотическая сложность каждого метода — это позволяет оценить применимость на больших датасетах.

**Результат:** сводная фигура — kriging vs ML на разных плотностях для каждого механизма зануления (heatmap), плюс отдельная таблица `method × stability_score × degradation_slope × ΔF1 × Moran_preservation × scaling_complexity`.
...
### D_privacy. Privacy-induced missingness

**Задача:** воспроизвести реальный механизм privacy-зануления данных ОФД/ФНС и оценить, как разные методы восстановления справляются с ним.

**Протокол на ФНС (основной):**
ФНС и ОФД зануляют не случайные пространственные зоны, а конкретные hex'ы по value-based критериям — чтобы скрыть либо данные отдельных крупных мерчантов, либо агрегаты при недостаточной k-anonymity. Эмулируем три таких сценария на основе анализа перцентилей по собранным данным:

1. **k-anonymity (мало мерчантов)** — зануляем поля для hex'ов с `kkt_count < p5`: слишком мало ККТ, чтобы агрегаты были анонимными.
2. **Privacy крупного бизнеса** — зануляем `avg_bill` для hex'ов с `avg_bill > p95`: аномально высокий чек указывает на одного крупного мерчанта.
3. **Концентрация выручки** — зануляем hex'ы, где `revenue_intensity` высокий, а `kkt_count` низкий: выручка концентрируется на 1–2 ККТ, усреднение раскрывает их оборот.

Восстанавливаем искусственно занулённые значения через все методы из бенчмарка. Сравниваем с естественными `null`'ами в данных (ФНС уже зануляет часть полей по тем же критериям) — это ground-truth проверка реалистичности эмуляции.

**Метрики:**
- MAE между восстановленными и истинными значениями на занулённых hex'ах
- Moran's I до и после восстановления — насколько сохраняется пространственная автокорреляция
- Сравнение методов по типам зануления — какой метод выигрывает на каком сценарии

**Протокол на RE (дополнительный, сравнение механизмов):**
Эмулируем cluster missingness (зануление точек в радиусе 1–5 км от случайных центров) на RE. Это **другой механизм**, не воспроизводящий privacy ОФД — но даёт сравнение: восстанавливается ли cluster-зануление так же хорошо, как value-based, или это структурно разные задачи.

**Результат:** ноутбук-демо `notebooks/privacy_demo.ipynb` с таблицей "метод × тип зануления → качество восстановления"; сравнительный анализ value-based vs cluster missingness.
...
| Spatial structure preservation | `Moran_I(imputed) / Moran_I(original)` | 1.0 = метод сохраняет пространственную автокорреляцию, <1 = сглаживает поле (типично для kriging) |
| Fit time | секунды на обучение метода на одном регионе | для эмпирической оценки масштабируемости |
| Transform time | секунды на инференс одной точки и батча в 1M точек | стоимость применения на больших данных |
...
- Размер блока для **spatial block CV** определяется через эмпирическую вариограмму target-колонки: ширина блока ≥ 2 × range вариограммы. Для areal data (BDMO, ФНС-hex) — блок = одна административная единица или H3-родитель на одну резолюцию грубее (резолюция 7 родитель для resolution 8)
